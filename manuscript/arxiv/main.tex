...
% --- Title block ----------------------------------------------------------
\title{RASD: Ring Attention with Speculative Decoding for\\
...
full forward pass through the model.  We present \textbf{RASD}
...
model into a single end-to-end loop.  On 8$\times$A100 80\,GB SXM4,
RASD runs Llama-2-7B inference at 1\,M-token context with 40\,GB
peak per rank --- a regime where vanilla HuggingFace
FlashAttention-2 \texttt{generate()} runs out of memory at 128\,k
single-rank.  At the model's native 4\,k context with real PG-19
narrative text, RASD delivers a \textbf{4.4$\times$ throughput
speedup} over a matched target-only baseline with median per-round
acceptance of \textbf{1.0} (all four draft tokens accepted in most
verify rounds).  At extended contexts under YaRN factor 256, the
speedup contracts to 1.0--1.8$\times$ and per-round acceptance
becomes bimodal --- a regime we attribute to Llama-2-7B going
out-of-distribution beyond its 4\,k training horizon, not to a limit
of the RASD architecture.  We release the full implementation, all
...
These two techniques have not been combined into a published
million-token inference system on commodity hardware.  Both Ring
Attention and speculative decoding exist as independent components,
each with mature open-source implementations.  But running them
together at 1\,M context surfaces practical questions that neither
literature alone answers: how does NF4 KV quantization interact with
the YaRN \citep{peng2023yarn} position-scaling required to push
Llama-2-7B past its 4\,k training horizon; how does the speculative
verify loop's broadcast-and-resample pattern compose with the ring
attention's existing collective; and most importantly, \emph{does the
...
At Llama-2-7B's native 4\,k context with real PG-19
\citep{rae2019pg19} narrative text, RASD delivers a
$\mathbf{4.4\times}$ throughput speedup over the target-only
baseline, with median per-round acceptance of $\alpha = 1.0$
(all four draft tokens accepted in most verify rounds).  At extended
contexts of 128\,k--1\,M tokens under YaRN scaling factors 32--256,
the speedup contracts to $1.0\text{--}1.8\times$, and per-round
$\alpha$ becomes bimodal: approximately half of verify rounds
fully reject the draft ($\alpha = 0$) while the remainder accept
1--2 of 4 draft tokens.  Profiler measurements show the
communication cost of the ring exchange is $\leq 1.2\%$ of wall
time on the speculator rank --- partially refuting the
\citet{liu2023ringattention} expectation that communication
masking is where the speedup must come from.  Instead, RASD's
contribution at long context is bounded by base-model acceptance,
which itself is bounded by the YaRN extrapolation regime: the
$4.4\times$ at 4\,k is the lower bound, not the long-context
$1.0\times$.

\paragraph{Contributions.}
\begin{itemize}[leftmargin=*,topsep=0pt]
  \item A 1\,M-token inference system that runs on 8$\times$A100
        80\,GB at 40\,GB peak per rank.  No prior open-source baseline
        we are aware of reaches 1\,M with this hardware configuration.
  \item An apples-to-apples speedup quantification: 4 contexts
        $\times$ 3 seeds, with a matched target-only baseline that
        differs from RASD in exactly one variable (spec\_steps).
        Mean speedups: 1.24$\times$ (128\,k), 1.80$\times$ (256\,k),
        0.97$\times$ (512\,k), 1.00$\times$ (1\,M).
  \item A characterization of when speculative decoding pays off at
        long context, anchored on per-round $\alpha$ traces.  We
        identify a bimodal $\alpha$ failure mode and link it
        mechanistically to YaRN-extrapolation entropy in the
        target's logits.  A short-context PG-19 ablation
        ($\alpha = 0.706$ at 4\,k) shows the long-context contraction
        is base-model-driven, not RASD-architecture-driven.
  \item An open-source reference implementation released under MIT,
        with version-pinned \texttt{requirements-lock.txt}, every
        per-position $\alpha$ trace, every generated continuation,
        per-rank memory snapshots, and per-context profiler JSONs.
\end{itemize}
...
The transformer architecture \citep{liu2023ringattention} established
...
Longformer \citep{liu2023ringattention} replaced full attention with a
sliding window of width $w$, reducing complexity to
$\mathcal{O}(N \cdot w)$ while adding global tokens to preserve
long-range reasoning. Transformer-XL \citep{liu2023ringattention}
...
windows during the decode phase.}
...
Ring Attention \citep{liu2023ringattention}    & \checkmark & \checkmark & --- & --- \\
Speculative Decoding \citep{leviathan2023fast} & --- & ---  & \checkmark & --- \\
Medusa \citep{cai2024medusa}                   & --- & ---  & \checkmark & --- \\
vLLM \citep{kwon2023vllm}                      & \checkmark & \checkmark & --- & --- \\
...
RASD is an inference loop that runs on $W=8$ A100 80\,GB SXM4 GPUs
on a single node. The target model (Llama-2-7B) and a smaller draft
model (Sheared-LLaMA-1.3B \citep{xia2023sheared}) are both loaded
on every rank, both quantized to NF4 4-bit weights via
bitsandbytes \citep{dettmers2022bitsandbytes}. The KV cache is also
NF4-quantized, with a bf16 prefix of the first 128 tokens held only
on rank 0. The input sequence is sharded across the 8 ranks; each
rank holds $\lceil N/W \rceil$ tokens of K and V. Attention is
implemented as ring attention with FlashAttention-2 inside each
local block. Position embeddings use YaRN with factor
...
ring exchange uses \texttt{torch.distributed.batch\_isend\_irecv}
...
factor is the NF4 4-bit compression ratio relative to bf16. At
$N = 10^6, W = 8$ this gives $M_{\text{rank}} \approx 4.0 + 32 + 4
= 40$\,GB, which we measure within rounding (\S\ref{sec:results-memory}).
...
(\texttt{tests/test\_verification\_math.py}) that runs the verify
...
\texttt{transformers} library (v4.46.3). We use
bitsandbytes 0.49.2 for NF4 quantization,
\texttt{flash-attn} 2.8.3 for the local-block kernel, and
\texttt{wandb} for run logging. All package versions are pinned in
\texttt{requirements-lock.txt} (frozen at the
\texttt{m4-phase-c-complete} git tag).

The repository ships the canonical aggregator
\texttt{scripts/aggregate\_final\_results.py} that builds
...
pods were used in sequence: a Phase C pod that ran the headline
matrix and a Phase D pod that captured per-position acceptance traces
and saved generations. Both pods used identical software stacks (per
the live-vs-locked \texttt{requirements-lock.txt} diff check at
bootstrap).

All ablation and matrix runs are publicly logged on Weights \&
Biases\footnote{\url{https://wandb.ai/amank-iitg-uc-berkeley-electrical-engineering-computer-s/rasd-m4-phase-c}
\;and\;
\url{https://wandb.ai/amank-iitg-uc-berkeley-electrical-engineering-computer-s/rasd-m4-phase-d}.}
under the run-id convention used in this paper. We cite specific run
slugs inline where useful (e.g., the p36 canary run is
\texttt{bxn758a9}), and the
Reproduction Appendix (\S\ref{app:repro}) maps every figure and
table to its source CSV and the corresponding wandb run.
...
\texttt{princeton-nlp/Sheared-LLaMA-1.3B} \citep{xia2023sheared},
revision \texttt{a4b76938edbf571ea7d7d9904861cbdca08809b4} (pinned).
Same NF4 weight quantization. The two models share a tokenizer.
...
We ran six experimental stages, each producing committed CSV+sidecar
artifacts in the repository.

\paragraph{Stage p33 / p34 (smoke + baseline validation).}
Long-context smokes at 16\,k--128\,k to validate the ring decode
loop; the M3 ablation grid baseline reproduces $\alpha$ from M3.

\paragraph{Stage p35 (RASD final matrix).}
4 contexts $\times$ 3 seeds = 12 cells, plus a canary.
$\gamma = 4$, $c = 2048$, $k$-block prefetch depth 1, YaRN.
Output: \texttt{results/final/final\_matrix.csv}.

\paragraph{Stage p35b (target-only baseline).}
Same 4 $\times$ 3 grid as p35, with $\gamma = 0$ (target-only
autoregressive decode). The same code path is exercised: ring
attention, NF4 KV, YaRN, outlier-keep. The \emph{only} difference is
the speculative verify loop is disabled. This is the
apples-to-apples baseline that all speedup numbers in this paper
reference.

\paragraph{Stage p35c (perplexity sanity).}
4 contexts $\times$ 3 seeds, single-rank evaluation of NF4 +
YaRN-extended Llama-2-7B on PG-19. We additionally ran a vanilla-RoPE
control at the same contexts (Appendix~\ref{app:vanilla-ppl}).

\paragraph{Stage p36 (profiler subset).}
4 contexts $\times$ 1 seed wrapped in \texttt{torch.profiler}. The
canary + 128\,k + 256\,k + 512\,k cells produced clean profile JSONs;
the 1\,M cell was attempted but \texttt{torch.profiler.key\_averages()}
event aggregation did not return in 2\,h of CPU work
(\S\ref{sec:limitations}).

\paragraph{Stage p37 (vanilla HF FA-2 ceiling).}
Single-rank vanilla HuggingFace \texttt{model.generate()} with
\texttt{attn\_implementation=flash\_attention\_2}, no NF4, no ring
SP. 5 contexts: $\{32, 128, 256, 512, 1024\}$\,k. Records the
out-of-memory ceiling of the standard inference stack.

\paragraph{Phase D rerun (per-token + generated text capture).}
The Phase C pod was terminated before we realized
\texttt{per\_token/*.jsonl} sidecars are gitignored (the rule was
written assuming long-context traces would be 100\,MB+; in practice
at $\gamma = 4$, max\_new\_tokens=64 they are 5\,KB). A second pod
re-ran the headline 8 cells (4 RASD + 4 target-only at each context)
plus 4 short-context PG-19 cells (4\,k and 8\,k for both modes) with
\texttt{--log-per-token --save-generated-text}. All sidecars are
committed.
...
\begin{itemize}[leftmargin=*,topsep=0pt]
    ...
    \item \textbf{Compute / comm / idle / other (\%)}: rank-0
          time-share buckets from \texttt{torch.profiler.key\_averages()}.
\end{itemize}
...
verify loop is removed. This is the p35b target-only baseline.
...
${\approx} 0.7$\,GB). The peak per-rank memory at 1\,M (40.3\,GB)
agrees with the memory equation in Eq.~\ref{eq:memory} within
rounding.
...
(\texttt{results/final/per\_token/<run\_id>.jsonl}, one record per
round with \texttt{n\_acc}, \texttt{global\_pos\_start},
\texttt{spec\_steps}, \texttt{draft\_tokens}). Figure~\ref{fig:alpha}
shows two views.
...
\texttt{torch.profiler.key\_averages()} aggregation became
...
We pin every dependency in \texttt{requirements-lock.txt} and run an
env-drift check on every pod bootstrap. Two reproducibility threats
remain: (i) Lambda Cloud's Ubuntu image versions can drift (the Phase
D pod's image was different from the Phase C pod's --- both
ran the same Python/CUDA but a future image bump could break things);
(ii) we depend on a specific revision of Sheared-LLaMA-1.3B
(\texttt{a4b76938edbf571ea7d7d9904861cbdca08809b4}) and a specific
release of bitsandbytes for NF4 numerics. We pin both. The released
...
We presented RASD, an inference system that combines ring-attention
sequence parallelism, NF4 KV-cache quantization with chunked updates
and a bf16 outlier-keep prefix, YaRN position scaling, and
speculative verification with a small draft model. On
8$\times$A100 80\,GB SXM4 the system runs Llama-2-7B inference at
1\,M-token context at 40\,GB peak per rank --- a regime where the
standard HuggingFace inference stack runs out of memory at 128\,k
single-rank. Against a matched target-only baseline we measure a
$4.4\times$ throughput speedup at the model's native 4\,k context
with real PG-19 text, contracting to $1.0\text{--}1.8\times$ at
extended contexts. We trace the contraction mechanistically to
Llama-2's out-of-distribution behavior under YaRN factor=256, not to
an architectural limit of RASD itself: short-context PG-19 ablations
and per-round acceptance analysis localize the rate-limiter to
target-logit entropy in the YaRN-extrapolated regime. We also
report a useful negative result: at $W = 8$ on NVLink, communication
cost is only 1.2\% of wall, so the comm-overlap budget the original
gap analysis predicted is not load-bearing; RASD's speedup at long
context is driven entirely by speculative amortization.
...
\begin{itemize}[leftmargin=*,topsep=0pt]
    \item \texttt{m3-complete} --- M3 ablation grid (Table~F2 heatmap source)
    \item \texttt{m4-phase-c-complete} --- the headline matrix
          (p35 / p35b / p35c / p36 / p37)
    \item \texttt{m4-phase-d-complete} --- per-position $\alpha$ traces,
          saved generations, short-context PG-19 ablation
\end{itemize}
Reproducing on a fresh 8$\times$A100 80\,GB SXM4 pod takes
${\sim}1.5$\,h end-to-end at \$22/h on Lambda; the full
\texttt{scripts/phase\_c\_pod\_session.sh} runbook bootstraps from
scratch with one command (\texttt{nohup bash …}).
...
\begin{table}[h]
\centering\small
\caption{Reproduction map. The wandb columns link to the public W\&B
runs (project URLs in \S\ref{sec:experiments-hardware}). $^\dagger$Run
on the Phase D pod with the same code and pinned versions.}
\label{tab:repro}
\begin{tabular}{lll}
\toprule
Paper artifact & Source CSV / sidecar & wandb run-id \\
\midrule
F1 throughput & \texttt{results/final/final\_matrix.csv} +
              & \texttt{M4\_ctx\{128k,256k,512k,1M\}\_s\{42,123,456\}} \\
              & \texttt{target\_only\_matrix.csv}
              & + \texttt{TARGET\_ctx*} \\
F1 HF ceiling rows & \texttt{results/baselines/hf\_ceiling.csv} & (no wandb; CSV-only) \\
F3 time breakdown & \texttt{results/final/profiler\_pass/profiler/*.json}
                  & \texttt{matrix\_profiled\_canary\_s42}\,(\texttt{bxn758a9}), \\
                  & & \texttt{PROFILED\_ctx\{128k,256k,512k\}\_s42} \\
F4 $\alpha$ vs round & \texttt{results/final/per\_token/*.jsonl}
                     & \texttt{RASD\_ctx*\_phaseD\_s42}$^\dagger$ \\
F5 qualitative & \texttt{results/final/generated/*.txt}
               & (Phase D runs, same slugs as F4) \\
Table~\ref{tab:positioning} & (literature review)
               & --- \\
Speedup table  & \texttt{results/final/final\_results.json}
               & (aggregated from above) \\
PPL table     & \texttt{results/perplexity/m4\_ppl.csv} (YaRN)
              & (no wandb; PG-19 eval is single-rank, no run log) \\
              & \texttt{m4\_ppl\_vanilla\_rope.csv} (control)
              & (no wandb) \\
\bottomrule
\end{tabular}
\end{table}
...
% Per-seed CSV dumps for p35, p35b, profiler matrix, PG-19 short-ctx.
