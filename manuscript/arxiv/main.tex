...
% =========================================================================
\begin{abstract}
Long-context inference for large language models faces two
compounding bottlenecks: the KV-cache memory and bandwidth cost at
million-token contexts exceeds the capacity of any single GPU, and
autoregressive decoding amortizes a single output token over a full
target-model forward pass. We present \textbf{RASD} (Ring Attention
with Speculative Decoding), an inference system that integrates
ring-attention sequence parallelism, NF4 KV-cache quantization with
chunked updates and a bf16 attention-sink prefix, YaRN position
scaling, and speculative verification with a small draft model into
a single end-to-end loop. On 8$\times$A100 80\,GB SXM4, RASD runs
Llama-2-7B inference at 1\,M-token context with 40\,GB peak per
rank --- a regime where the standard open-source inference stack
(HuggingFace FlashAttention-2) runs out of memory at 128\,k
single-rank. We derive the per-rank memory equation, validate it
within rounding across the matrix, and show that the chunked-NF4
KV update is what brings the 1\,M case inside the available
per-rank budget.
We complement the capability result with a three-seed apples-to-
apples speedup matrix against a target-only baseline that differs
from RASD in exactly one variable (speculative steps disabled):
RASD attains a 1.24--1.80$\times$ throughput speedup at moderate
contexts (128\,k--256\,k) and converges to parity at 512\,k and
1\,M. Per-round acceptance becomes bimodal at long context ---
about half of verify rounds fully reject the draft --- and a
short-context control on real text confirms this is base-model
behavior under extreme YaRN extrapolation, not an RASD limitation.
A profiler measurement at four contexts shows communication is
${\leq}1.2\%$ of wall on the speculator rank, partially refuting
the hypothesis that overlap between speculative work and ring
communication is the mechanism behind the speedup. We release the
implementation under the MIT license with version-pinned
dependencies, every per-position acceptance trace, and the full
M3 ablation grid that derived the system's design point.
\end{abstract}
...
We report the headline numbers up-front so readers can place the
rest of the paper in context. On 8$\times$A100 80\,GB SXM4, RASD
reaches 1\,M-token context with a peak per-rank memory of 40\,GB
--- half the available 80\,GB. In the same hardware configuration,
HuggingFace's standard \texttt{Llama-2-7b-hf.generate()} with
FlashAttention-2 runs out of memory at 128\,k single-rank, so a
direct throughput comparison at long context is not well-defined;
we therefore quantify speedup against a matched target-only
baseline that uses the same RASD infrastructure but disables the
speculative verify loop. The three-seed mean speedup is
$1.24\times$ at 128\,k, $1.80\times$ at 256\,k, and converges to
parity ($0.97\times$, $1.00\times$) at 512\,k and 1\,M. A
profiler measurement on the speculator rank shows that
communication is $\leq 1.2\%$ of wall across the matrix, partially
refuting the hypothesis that comm-overlap is the mechanism behind
the speedup. Per-round acceptance at long context is
\emph{bimodal} (about half of rounds reject the entire draft);
a short-context control on real text shows this bimodality is
specific to Llama-2 under YaRN factor 256 and is not present at
the model's native training context.
...
\begin{itemize}[leftmargin=*,topsep=0pt]
  ...
  \item \textbf{An honest characterization} of when speculative
        ...
        to base-model behavior under YaRN factor 256, and document
        a useful negative result: the communication-masking
        hypothesis behind the original RASD design is not
        load-bearing on 8 ranks over NVLink.
...
Figure~\ref{fig:rasd-arch} illustrates the RASD system at the level
of one decode round. RASD is an inference loop that runs on $W = 8$
...
\begin{tikzpicture}[
  ...
  comp/.style={font=\scriptsize, align=center},
  ring/.style={-Latex, line width=0.6pt, blue!50!black},
  flow/.style={-Latex, line width=0.9pt, dashed, gray!70},
  >=Latex,
]

% Panel (a): ring topology
\begin{scope}
\node at (0,3.0) {\textbf{(a) Ring topology of 8 ranks at one decode round}};
...
\foreach \i [count=\n from 0] in {0,45,90,135,180,225,270,315} {
  ...
  \ifnum\rid=0
    \node[rank0] (r\rid) at (\px,\py) {%
      \textbf{Rank 0}\\[2pt]
      \scriptsize Target NF4\\
      \scriptsize \textbf{Draft NF4}\\
      \scriptsize \textbf{bf16 prefix [128]}\\
      \scriptsize KV shard NF4\\
      \scriptsize $\lceil N/8 \rceil$ tok
    };
  \else
...
\caption{RASD architecture. (a) Sequence is sharded across $W = 8$
ranks; each holds $\lceil N/W \rceil$ tokens of NF4-quantized K and V.
Rank 0 additionally holds the draft model and a bf16
attention-sink prefix of the first 128 tokens. The
ring exchange uses \texttt{batch\_isend\_irecv} collectives and
FlashAttention-2 inside each local block; per-round per-device memory
is $\mathcal{O}(N/W)$, independent of total sequence length. (b) One
decode round: the draft proposes $\gamma{=}4$ tokens on rank 0, the
target verifies them in a single packed forward over $\gamma{+}1$
positions on all 8 ranks via ring attention, target logits are
broadcast from rank 0 so every rank can sample identically using a
shared RNG seed, and the longest matching prefix is accepted
