...
\begin{itemize}[leftmargin=*,topsep=0pt]
  ...
  \item An open-source reference implementation released under MIT,
        with version-pinned \path{requirements-lock.txt}, every
...
\path{princeton-nlp/Sheared-LLaMA-1.3B} \citep{xia2023sheared},
revision \path{a4b7693} (40-character hash pinned in
\path{requirements-lock.txt}; abbreviated here). Same NF4 weight
quantization. The two models share a tokenizer.
...
the 1\,M cell was attempted but \path{torch.profiler.key_averages()}
event aggregation did not return in 2\,h of CPU work
(\S\ref{sec:limitations}).

\paragraph{Stage p37 (vanilla HF FA-2 ceiling).}
Single-rank vanilla HuggingFace \path{model.generate()} with
\path{attn_implementation=flash_attention_2}, no NF4, no ring
...
\path{per_token/*.jsonl} sidecars are gitignored (the rule was
