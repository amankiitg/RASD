...
1.24--1.80$\times$ at moderate contexts (128\,k--256\,k) to
$0.96\times$ at 512\,k and parity ($1.00\times$) at 1\,M.
...
SXM4, RASD runs Llama-2-7B at 1\,M-token context with 40\,GB
...
\node[draftstep] (a) at (0, 2.6)
  {\textbf{1.\,Draft generates $\gamma{=}4$ tokens} \, (rank 0 only)};
...
\item \textbf{Throughput matrix} (Tables~\ref{tab:speedup},
  ...
  target-only baseline). 95\% bootstrap CIs in
  \path{results/final/ablation_cis.csv}.
...
\item \textbf{M3 ablation grid} (Fig.~\ref{fig:ablation},
  ...
  \path{results/ablations/r65_audit.md}). This grid corresponds to
  the \path{m3-reablation} git
